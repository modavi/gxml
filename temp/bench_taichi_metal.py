"""
Benchmark comparing CPU vs Taichi GPU (Metal) solvers.
Test different Taichi architectures.
"""
import sys
import time
...
def benchmark_arch(arch_name: str, iterations: int = 10):
    """Benchmark with specific Taichi architecture."""
    import taichi as ti
    
    if arch_name == 'cpu':
        ti.init(arch=ti.cpu)
    elif arch_name == 'metal':
        ti.init(arch=ti.metal)
    elif arch_name == 'vulkan':
    ...
    for _ in range(3): run_pipeline(XML_75_PANELS)  # warmup
    
    cpu_times = []
    for i in range(iterations):
        start = time.perf_counter()
        run_pipeline(XML_75_PANELS)
        cpu_times.append((time.perf_counter() - start) * 1000)
    
    cpu_avg = sum(cpu_times) / len(cpu_times)
    print(f"  CPU Solver: {cpu_avg:.2f}ms avg (min={min(cpu_times):.2f}, max={max(cpu_times):.2f})")
    
    # Test Taichi GPU solver
    ...
    print(f"  GPU Solver ({arch_name}): {gpu_avg:.2f}ms avg (min={min(gpu_times):.2f}, max={max(gpu_times):.2f})")
    
    if gpu_avg < cpu_avg:
        print(f"  -> Taichi {arch_name.upper()} is {cpu_avg/gpu_avg:.2f}x FASTER")
    else:
        print(f"  -> Taichi {arch_name.upper()} is {gpu_avg/cpu_avg:.2f}x SLOWER")
    
    return cpu_avg, gpu_avg

def main():
    print("GXML Taichi Backend Benchmark")
    print("=" * 50)
    print("Test: 75 panels with rotations/attachments")
    
    # Try Metal first (macOS GPU)
    try:
        cpu_avg, metal_avg = benchmark_arch('metal', iterations=10)
        print(f"\n[Metal GPU] {cpu_avg:.2f}ms CPU vs {metal_avg:.2f}ms GPU")
    except Exception as e:
        print(f"\nMetal not available: {e}")
        # Fall back to CPU backend (which still shows Taichi overhead)
        cpu_avg, taichi_cpu_avg = benchmark_arch('cpu', iterations=10)
        print(f"\n[Taichi CPU] {cpu_avg:.2f}ms CPU vs {taichi_cpu_avg:.2f}ms Taichi")
