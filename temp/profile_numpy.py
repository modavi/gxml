"""Profile numpy.array allocations."""
import sys
...
def counting_array(*args, **kwargs):
    ...
    # Get the caller (skip counting_array itself)
    for frame in reversed(stack[:-1]):
        if 'gxml' in frame.filename:
            key = f"{frame.filename.split('gxml/')[-1]}:{frame.lineno}"
...
# Sort and print
print("numpy.array calls by location:")
for k, v in sorted(call_counts.items(), key=lambda x: -x[1])[:25]:
    print(f"  {v:6d}  {k}")
